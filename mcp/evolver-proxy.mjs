 ...
 * tools so Antigravity can search/reuse/publish evolution assets natively.
 *
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if ([401, 403].includes(res.status)) {
        hint = token
          ? ' The Proxy token in ~/.evolver/settings.json looks stale (the Proxy mints a fresh token on restart). Restart this Antigravity session so the bridge re-reads it, or run /status.'
...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Distill a reusable Gene/Capsule from the current Antigravity conversation. Provide a concrete summary, strategy/evidence, artifacts, and validation; the Proxy gates quality, stores locally, and queues Hub publishing.',
    inputSchema: {
      type: 'object',
      properties: {
        title: { type: 'string' },
        summary: { type: 'string', description: 'Concrete reusable lesson or capability distilled from the conversation.' },
        platform: { type: 'string', default: 'antigravity' },
    ...
    handler: (a) => proxyFetch('POST', '/conversation/distill', { ...a, platform: a.platform || 'antigravity' }),
  },
  {
    name: 'evolver_poll',
