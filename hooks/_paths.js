...
//
// Shared path / workspace helpers for the Evolver Antigravity plugin hooks.
 ...
 *   3. ANTIGRAVITY_PROJECT_DIR   (Antigravity Desktop project dir, if set)
...
function resolveProjectDir() {
  ...
  const fromAntigravityDir = process.env.ANTIGRAVITY_PROJECT_DIR || process.env.AGY_PROJECT_DIR;
  if (looksLikeDir(fromAntigravityDir)) {
    return fromAntigravityDir;
  }
 ...
 *   - ANTIGRAVITY_WORKSPACE / AGY_WORKSPACE wins if set.
...
function computeWorkspaceRoot(projectDir) {
  ...
  const fromAntigravity = process.env.ANTIGRAVITY_WORKSPACE || process.env.AGY_WORKSPACE;
  if (typeof fromAntigravity === 'string' && fromAntigravity.length > 0) {
    return fromAntigravity;
  }
