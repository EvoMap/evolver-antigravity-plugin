...
//
// Antigravity hook: PostToolUse (Write|Edit|MultiEdit|write_to_file|replace_file_content|multi_replace_file_content).
...
function extractContent(input) {
  ...
  const ti = input.tool_input || input.arguments || input;
  if (ti && typeof ti === 'object') {
    ...
    if (typeof ti.CodeContent === 'string') return ti.CodeContent;
    if (typeof ti.ReplacementContent === 'string') return ti.ReplacementContent;
    if (Array.isArray(ti.ReplacementChunks)) {
      return ti.ReplacementChunks.map(chunk => chunk.ReplacementContent || '').join('\n');
    }
  }
